#!/usr/bin/env bash
# SessionStart-hook: en kort pekare till webapp-kit-pluginets ingångar.
...
# upp för pluginet, där de filerna inte finns. Första raden i sessionen blev alltså en
...
pluginets egen vokabulär; översätt dem alltid till vanlig svenska när du talar med
...
- OBS: det här projektet är INTE uppsatt för pluginet (ingen Project_state.md). Läs alltså inte
  doc-roll-filerna — de finns inte. Flera skills förutsätter dem och blir tunnare utan.
  Vet användaren inte var hen ska börja → kom-igang-skillen (introducerar pluginet i klartext
